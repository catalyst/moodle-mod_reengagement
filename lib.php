...
function reengagement_crontask() {
    ...
    foreach ($inprogresses as $inprogress) {
        ...
        if (empty($completionrecord)) {
            mtrace("Could not find completion record for updating to complete" .
              " state - userid: $userid, cmid: $cmid - recreating record.");
            ...
            if (!empty($userfielddata[$field->id])) {
                if ($field->datatype == 'datetime') {
                    ...
                    $templatevars['/%profilefield_'.$field->shortname.'%/'] =
                      userdate($userfielddata[$field->id]->data, $format);
                } else {
                    $templatevars['/%profilefield_'.$field->shortname.'%/'] =
                      format_text($userfielddata[$field->id]->data,
